...
/**
 * 2.直销交易账号拆分：DS-mid_agrm_tradeaccount_ds
 */
public class SparkDSAgencySplit {
    ...
    public static void main(String[] args) {
        ...
        String sql_data =
                "SELECT  sk_account_of_ds as trd_st_trx_serialno " +
                        ...
                        "  FROM "+pi_schema+".mid_agrm_tradeaccount_ds t " +
        ...
        String sql_mapp = "select t.bk_agency, t.sk_agency, t.effective_from, t.effective_to  from "+pi_schema+".ip_agency_mapp t";
        System.out.println("================sql_mapp====================");
        System.out.println(sql_mapp);
        System.out.println("=================sql_mapp===================");
        Dataset<Row> ddf = hc.sql(sql_mapp);

        //final Row[] rules = rdf.collect();
        ...
        JavaRDD<Row> w = ipdf.toJavaRDD().map(new Function<Row, Row>() {
            ...
            public Row call(Row r) throws Exception {
                ...
                for (int i = 0; i < rules.length; i++) {
                    ...
                    //rule_id=new BigDecimal((String) rules[i].getAs("ag_split_ruleno"));
        ...
        StructType schema = hc.sql("select "
                + "cast(0 as decimal(12,0)) as src_id"
                + ",cast(0 as decimal(12,0)) as sk_agency"
                + ",cast(0 as string) as rule_id")
                .schema();
        hc.createDataFrame(w, schema).write().format("parquet").mode(SaveMode.Overwrite).saveAsTable(pi_schema+"."+pi_tgtTableName);
        hc.close();
    }
