    ...
    public static void main(String[] args) {
        ...
        String sql_schema =
                "SELECT t.trd_st_trx_serialno,\n" +
                        ...
                        "  FROM " + schema + ".Trd_Ta_Saletran_Arsplt t\n" +
                        " WHERE 1 = 2";

        System.out.println("================sql_schema====================");
        System.out.println(sql_schema);
        System.out.println("=================sql_schema===================");
        StructType st_old = hc.sql(sql_schema).schema();
        String sql_data =
                "SELECT         t.trd_st_trx_serialno,\n" +
                        "       cast(-1 as decimal(12,0)) AS sk_invpty_type,\n" +
                        "       f.sk_invpty_of_cust       AS sk_invpty,\n" +
                        ...
                        "  LEFT JOIN  " + schema + ".agrm_fundaccount f\n" +
                        "    ON t.sk_account_of_fd = f.sk_account_of_fd\n" +
                        ...
                        //TODO 添加交易成功的条件-20201130 已经添加
                        "   AND dk_saletran_status='1' "
                        ;
        ...
        String sql_mapp = "WITH a AS\n" +
                ...
                + " )\n" +
                " select concat_ws(',',concat_ws('.',dk_tano,cserialno), cast(sk_account AS string),cast(sk_invpty AS string),dk_cust_type,cast(sk_product AS string),cast(sk_agency_of_lv1 AS string),cast(nvl(sk_region_of_lv1,-1) AS string)) AS KEY, \n" +
                "       t.area_rule_id,\n" +
                "       t.sk_product_type,\n" +
                "       t.sk_product,\n" +
                "       t.dk_cust_type,\n" +
        ...
        String schema_sql = "SELECT cast(0 as decimal(30,0)) as trd_st_trx_serialno,\n" +
                "		cast(null as string) as dk_tano,\n" +
                "		cast(null as decimal(8,0)) as ta_cfm_date,\n" +
                "		cast(null as string) as dk_cust_type,\n" +
                "       cast(null as decimal(10,0)) as area_rule_id,\n" +
                "       cast(null as string) as dk_org_tree_of_brnch,\n" +
                "       cast(null as string) as sk_invpty_of_brnchorg,\n" +
                "       cast(null as string) as dk_org_tree_of_bl,\n" +
                "       cast(null as string) as sk_invpty_of_blorg,\n" +
                "       cast(null as string) as dk_custmngr_type,\n" +
                "       cast(null as string) as sk_invpty_of_custmngr,\n" +
                "       cast(null as string) as sk_product,\n" +
                "       cast(null as string) as div_ratio, \n" +
                "       cast(null as string) as dk_system_of_sdata \n";
        StructType st = hc.sql(schema_sql)
                .schema();
        hc.createDataFrame(x, st_old).write()
