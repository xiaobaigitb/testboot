 ...
 * 渠道拆分 spark2
 * 参数顺序：
 * 第一个参数：pi_srcsys 源，例如：HSTA4
 * 第二个参数：pi_batchno 批次，当前跑批的数据
 * 第三个参数：pi_dsagencyno DS的渠道代码，例如：237
 * 第四个参数：pi_tgtTableName   拆分出来的结果表,例如：spark_pl22
 * 第五个参数：pi_schema    库名
 * 要求：
 * 1.表
 * 1.1数据表-sql_lookdf22
 * ods.mid_trd_ta_saletran
 * 1.2 规则数据
 * ods.ip_agency_split_param
 * 1.3 bk_agency,sk_agency 对应关系表
 * ods.ip_agency_mapp
 */
public class SparkAgencySplit {
    ...
    public static void main(String[] args) {
        ...
        SparkSession hc = SparkSession.builder()
                .config(conf)
                .enableHiveSupport()
                .appName("SparkAgencySplit")
                //.master("local[4]")
                //.master("yarn-cluster")
                .getOrCreate();

        final String pi_srcsys = args[0];
        String pi_batchno = args[1];
        String pi_dsagencyno = args[2];
        String pi_tgtTableName = args[3];
        String pi_schema = args[4];


        String sql_data =
                "SELECT trd_st_trx_serialno " +
                        "      ,agencyno_split  as agencyno" +
                        "      ,requestno_split  as requestno" +
                        "      ,areacode_split  as areacode" +
                        "      ,netno_split  as netno" +
                        "      ,marketioflag " +
                        "      ,bk_tradeaccount AS tradeaccount " +
                        "      ,NULL AS bk_region_of_trd " +
                        "      ,NULL AS sk_agency_of_trd " +
                        "      ,sk_agency_of_ds_acct " +
                        "      ,null as capitalmode " +
                        "      ,ta_cfm_date AS sk_date " +
                        "      ,nvl(open_date,cast(19000101 as decimal(8,0))) AS open_date_of_trdacct " +
                        "      ,bk_region " +
                        "      ,'" + pi_dsagencyno + "' AS ds_agencyno " +
                        "  FROM " + pi_schema + ".mid_trd_ta_saletran t " +
        ...
        String sql_rule = "select t1.ag_split_ruleno " +
                ...
                "              ,t1.effective_from " +
                "              ,t1.effective_to " +
                "          from " + pi_schema + ".ip_agency_split_param t1 " +
        ...
        String sql_mapp = "select t.bk_agency" +
                ", t.sk_agency" +
                ", t.effective_from" +
                ", t.effective_to  " +
                "from " + pi_schema + ".ip_agency_mapp t";
        System.out.println("================sql_mapp====================");
        System.out.println(sql_mapp);
        System.out.println("=================sql_mapp===================");
        Dataset<Row> ddf = hc.sql(sql_mapp);

        final Row[] rules = (Row[]) rdf.orderBy("priority").collect();
        System.out.println("=====================================");
        for (int i = 0; i < rules.length; i++) {
            String s = rules[i].toString();
            System.out.println("规则:" + i + "-" + s);
        }
        System.out.println("=====================================");
        ...
        JavaRDD<Row> w = ipdf.toJavaRDD().map(new Function<Row, Row>() {
            ...
            public Row call(Row r) throws Exception {
                ...
                String agencyno = r.getAs("agencyno"); //005
                ...
                for (int i = 0; i < rules.length; i++) {
                    ...
                    if ("T_L".equals(rulename)) {
                        ...
                        if (rule1 != null) {
                            ...
                            field = splits[0].toLowerCase(); //Agency
                        ...
                        if (rule4 != null) {
                            ...
                            field = splits[0].toLowerCase(); //$$BK_REGION
                            splits = splits[1].split(","); //1,7
                            start = Integer.parseInt(splits[0]) - 1; //0
                            len = Integer.parseInt(splits[1]) + start; //7
        ...
        StructType schema = hc.sql("select cast(0 as decimal(12,0)) as src_id"
                + ",cast(0 as decimal(12,0)) as sk_agency,cast(null as string) as rule_id,cast(null as string) as srcsys")
                .schema();
        hc.createDataFrame(w, schema).write().format("parquet").partitionBy("srcsys").mode(SaveMode.Overwrite).saveAsTable(pi_schema + "." + pi_tgtTableName);
        hc.close();
    }
