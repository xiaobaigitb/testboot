...
import cn.com.businessmatrix.utils.JavaPairFlatMapPartitionFunc;
 ...
 * 第三步骤：对份额减少情况进行处理
 * 参数顺序：
 * 第一个参数：srcsys 源，例如：HSTA4
 * 第二个参数：sparktable 拆分出来的结果表,例如：spark_pl22
 * 第三个参数：startdate 开始日期
 * 第四个参数：enddate   结束日期
 * 第五个参数：schema    库名
 * 第六个参数：sparkname 程序ID
 * 要求：
 * 1.表
 * 1.1数据表-sql_lookdf22
 * ods.trd_ta_saletran
 * ods.comm_mkt_tradetype
 * ods.prod_asset_product
 * ods.ass_ta_cust_income_detail
 * ods.ass_ta_cust_income_detail_et
 * <p>
 * trd_ta_saletran
 * comm_mkt_tradetype
 * prod_asset_product
...
public class SparkPL22 {
    ...
    private static String format = sdf.format(new Timestamp(System.currentTimeMillis()));
    private static Timestamp timestamp = Timestamp.valueOf(format);
    ...
    public static void main(String[] args) throws Exception {
        ...
        hc.createDataFrame(x, lookdf.schema()).write()
                .format("parquet").partitionBy("dk_system_of_sdata").mode(SaveMode.Overwrite)
